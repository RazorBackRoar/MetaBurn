...
- When rebuilding for the user to try: bump `Sources/MetaBurn/Resources/version.json` (patch), then `razorbuild` / `./scripts/build-mac.sh`. Output: `build/Release/MetaBurn.dmg`. Open that DMG yourself to install. Human UAT (notification permission, launch-at-login, sleep/wake) still happens before a GitHub Release. `scripts/open-dmg.sh` only if they explicitly ask to open it. Keep the locked 500×420 DMG layout. DMG window background is a merged deep-red and dark-navy gradient matched to the MetaBurn icon (crimson throughout with dark blue, not red only at the top).
- Fire fills the whole window as a deep dark-red background (flames throughout, not a bottom strip). UI chrome stays readable in Auto, Light, and Dark. No custom cursor or cursor-fire trail. Open Files stays solid red.
