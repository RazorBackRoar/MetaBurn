...
- When rebuilding for the user to try: bump `Sources/MetaBurn/Resources/version.json` (patch), build in-repo (`build/Release/MetaBurn.dmg`), copy to `~/Desktop/MetaBurn <version>.dmg` (keep prior versioned DMGs) and replace `~/Desktop/MetaBurn.dmg`, then mount `MetaBurn.dmg` on the Desktop (`hdiutil attach`) and back up the current `/Applications/MetaBurn.app` to `~/Desktop/MetaBurn backup.zip`. Do not launch the `.app`, do not drag it into `/Applications`. The user drags the new `.app` from the mounted volume into `/Applications`, runs a short UAT pass (visual interaction, notification permission, launch-at-login, sleep/wake), and ejects manually. Never open a DMG twice. `scripts/open-dmg.sh` only if they explicitly ask to open it. Keep the locked 500×420 DMG layout.
